...
#Returns the movies who was created a little while ago
      return jsonify({
         'success': True,
         'status': 200,
...
#Abort 400 if any fields are missing
             abort(400, 'name, age and gender and idactor are required fields :( Please fill it out ^ـ^')

#Create a new actors and insert
          new_actors = Actor(name=name, age=age, gender=gender, id_actor=idactor )
          new_actors.insert()

#Returns the actors who was created a little while ago
          return jsonify({
          'success': True,
          'status': 200,
...
#PATCH/actors/<id>:
#Require the patch actors permission with data representation that will update actors if it exists.
#Returns status code 200 and json or the error handler.
...
#Bad Request
#Errorhandler when check the body request and status code 400
